...
minor_blues <- c(3 ,2 ,1 ,1 ,3, 2) #steps for constructing relative minor scale
...
if (index > 9) {
  ...
  key_tones <- c(index,index-9,index-7,index-6, index-5, index-2)
}else {root_relative_major_scale <- index+3

if (index %in% c(1,2)){
key_tones <- c(index,index+3,index+5,index+6, index+7, index+10)
} else if(index >= 3 & index <= 5){
key_tones <- c(index,index+3,index+5,index+6, index+7, index-2)
}else if(index == 6){
key_tones <- c(index,index+3,index+5,index+6, index-5, index-2)
}else if(index == 7){
key_tones <- c(index,index+3,index+5,index-6, index-5, index-2)
}else if(index %in% c(8,9)){
key_tones <- c(index,index+3,index-7,index-6, index-5, index-2)
}
}
}
#major scale, get relative minor and key chord tones
if (tone == "dom"){
  ...
  if (index > 1 & index <=5){
    ...
    key_tones <- c(index,index-8,index-5,index-2)
  } else 
    key_tones <- c(index,index+4,index+7,index+10)
}
#Blues scale
if (tone == "halfdim"){
  if (index > 9) {
    ...
    key_tones <- c(index,index-9,index-7,index-6, index-5, index-2)
         ...
         if (index %in% c(1,2)){
           key_tones <- c(index,index+3,index+6, index+10)
         } else if(index >= 3 & index <= 5){
           key_tones <- c(index,index+3,index+6, index-2)
         }else if(index == 6){
           key_tones <- c(index,index+3,index+6, index-2)
         }else if(index == 7){
           key_tones <- c(index,index+3,index-6, index-2)
...
while(sum_dur < (as.integer(struct[2]))) #while does not end section A keep generating notes/silence and duration
{
  ...
  {
    if(tone == "min"){
      i <- round(runif(1,1,6)) #pick scale key tones randomly  
    }else{
...
chords <- data.frame( chord1 = c("C","dom"),chord2=c("C","dom"),chord3=c("C","dom"),chord4=c("C","dom"),chord5=c("F","dom"),chord6=c("F","dom"),chord7=c("C","dom"),chord8=c("C","dom"),chord9=c("G","dom"),chord10=c("F","dom"),chord11=c("C","dom"),chord12=c("G","dom"))
...
key <- chords[1,1]
tone <- chords[2,1]
struct <- c("AABABA",1,1,0)#1 bar
melody.final <- melody(key, tone, struct, max_note_per_beat, harmony_first=0, harmony_in, rythm = 0)

#other chords
for(chord in 1:ncol(chords)){
  key <- chords[1,chord]
  tone <- chords[2,chord]
  struct <- c("AABABA",1,1,0)#1 bar
  melody.final <- cbind(melody.final, melody(key, tone, struct, max_note_per_beat, harmony_first=0, harmony_in, rythm = 0))
}
...
wait(play(sample))
s <- saveSample(sample,"music-22-CBlues-12bar.wav")
